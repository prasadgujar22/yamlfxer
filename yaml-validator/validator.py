...
def fix_colon_spacing(text: str, result: ValidationResult) -> str:
    """
    Normalize whitespace around a mapping key's colon to exactly ``key: value``.

    Handles two cosmetic problems that still parse as valid YAML, so they would
    otherwise go unnoticed:

    * a space *before* the colon  — ``name : John``  (W007)
    * irregular space *after* the colon — ``name:   John`` / ``name:\\tJohn`` (W008)

    The *missing* space case (``name:John``) is intentionally left to
    :func:`fix_missing_space_after_colon` (W005), which is URL-aware. To avoid
    mangling values that merely contain a colon (e.g. ``url: http://x``), only
    the key's separator colon is touched, and only when it is a genuine mapping
    colon (followed by whitespace or end-of-line).
    """
    protected = _protected_lines(text)
    lines = text.split("\n")
    # indent, optional "- " sequence prefix(es), key, spaces?, colon, spaces?, rest
    pattern = re.compile(
        r'^(?P<indent>\s*)(?P<dash>(?:-\s+)*)'
        r'(?P<key>"[^"]*"|\'[^\']*\'|[\w\-\.]+)'
        r'(?P<before>[ \t]*):(?P<after>[ \t]*)(?P<rest>.*)$'
    )
    before_count = 0
    after_count = 0
    for idx, line in enumerate(lines):
        ...
        if not m:
            continue
        before, after, rest = m.group("before"), m.group("after"), m.group("rest")
        # Only a genuine mapping colon (followed by whitespace or end-of-line).
        # "key:value" and "http://x" have a non-space char right after the colon
        # and are left to W005 / left alone.
        is_mapping_colon = len(after) >= 1 or rest == ""
        if not is_mapping_colon:
            continue
        bad_before = before != ""
        bad_after = rest != "" and after != " "
        if not (bad_before or bad_after):
            continue
        new_after = " " if rest != "" else ""
        prefix = m.group("indent") + m.group("dash")
        lines[idx] = f"{prefix}{m.group('key')}:{new_after}{rest}"
        keycol = len(m.group("indent")) + len(m.group("dash")) + 1
        if bad_before:
            before_count += 1
            result.issues.append(Issue(
                line=idx + 1,
                ...
                severity="warning",
                code="W007",
                message=f"Space before colon in key '{m.group('key')}'",
                suggestion="Remove the space before the colon, e.g. 'key: value'",
            ))
        if bad_after:
            after_count += 1
            result.issues.append(Issue(
                ...
                severity="warning",
                code="W008",
                message=f"Irregular spacing after colon in key '{m.group('key')}'",
                suggestion="Use exactly one space after the colon, e.g. 'key: value'",
            ))
    if before_count:
        result.corrections_made.append(
            f"Removed space before colon in {before_count} key(s)"
        )
    if after_count:
        result.corrections_made.append(
            f"Normalized spacing after colon in {after_count} key(s)"
        )
    return "\n".join(lines)
...
def fix_missing_colon(text: str, result: ValidationResult) -> str:
    ...
    candidates: dict[int, tuple[str, str, int]] = {}  # idx → (replacement, key, keycol)
    ...
    for idx, line in enumerate(lines):
        ...
        # A "- key: value" item opens an inline mapping; its keys (and any
        # colon-less siblings indented beneath it) share a group anchored at
        # this line with the inner key column as their indent.
        sm = seq_map_re.match(line)
        if sm:
        ...
        if not m:
            continue
        replacement = f"{' ' * indent}{m.group(2)}: {m.group(4)}"
        candidates[idx] = (replacement, m.group(2), len(m.group(1)) + 1)
        group_of[idx] = group
    ...
    for idx in sorted(to_fix):
        replacement, key, keycol = candidates[idx]
