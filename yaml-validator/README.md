...
- Irregular spacing around the colon (`name : John` / `name:   John` → `name: John`)
- **Missing colon entirely** (`name John` → `name: John`) — context-aware, see note below
  (also covers sequence-item mappings, e.g. `- a: 1` / `b 2` → `b: 2`)
...
| `W007` | warning | Space before colon (`name : John`) |
| `W008` | warning | Irregular spacing after colon (`name:   John`) |
